...
export async function GET(req: Request) {
  try {
    ...
    const cleanerResponse = await fetch(
      `${baseUrl}/api/catalog/fix-brands?offset=${currentOffset}`,
      {
        method: 'GET',
        cache: 'no-store',
      }
    ...
    const rateLimited = cleanerResult.rateLimited === true;

    const nextOffset = rateLimited
      ? currentOffset
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        ...
        last_processed: Number(cleanerResult.processed || 0),
        last_updated: Number(cleanerResult.updated || 0),
        last_unresolved: Number(cleanerResult.unresolved || 0),
    ...
    return NextResponse.json({
      ...
      cleaner: {
        ...
        suspiciousFound: cleanerResult.suspiciousFound,
